...
export class PokemonsService {
  ...
  getPokemons(): Observable<DonPokemon<Pokemon>> {
    return this.httpClient.get<DonPokemon<Pokemon>>(this.urlpokemon).pipe(tap(_ => this.log('Fetched pokemons')),
      catchError(this.handleError<DonPokemon<Pokemon>>('getPokemons', ))
    );
  }
  ...
  getPokemon(id: number): Observable<Pokemon> {
    ...
    return this.httpClient.get<Pokemon>(url).pipe(tap( _ => this.log('fetched hero id=${id}' )),
