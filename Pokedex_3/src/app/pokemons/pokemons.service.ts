...
export class PokemonsService {
  ...
  /*searchPokemon(search: string, offset: number, limit: number): Observable<DonPokemon<Pokemon>> {
    console.log(this.urlpokemon + '?search=' + search + '&limit=20' + '&offset=' + offset);
    return this.httpClient.get<DonPokemon<Pokemon>>(this.urlpokemon + '?search=' + search + '&offset' + offset + '&limit=' + limit).pipe(tap( _ => this.log('fetched pokemon' )),
      catchError(this.handleError<DonPokemon<Pokemon>>('get pokemon')));

  }*/

  searchPokemons(search: string, nbPokemon: number, limit: number): Observable<DonPokemon<Pokemon>> {
    const url = this.urlpokemon + '?search=' + search + '&offset=' + nbPokemon + '&limit=' + limit;
    return this.httpClient.get<DonPokemon<Pokemon>>(url).pipe(
